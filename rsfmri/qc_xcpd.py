...
def run_participant_qc(config, subject, session, job_ids=None):
    """
    Run QC on XCP-D outputs for a given subject and session.
...
def metric_extraction(config, subject, session):
    """
    Extract QC metrics for a single subject/session from XCP-D outputs.
    ...
    - Loads the XCP-D QC metrics file (TSV format) and extracts key metrics such as max framewise displacement (FD),
      max rotation and translation values, max RMSD
...
def metric_concatenation(config):
    """
    Concatenate per-subject/session XCP-D QC CSV files into a single group-level CSV.

    Concatenates available participant-level files into:
      - `DERIVATIVES_DIR/qc/xcpd/group_minimal_qc.csv` (in-house QC)
      - `DERIVATIVES_DIR/qc/xcpd/group_xcpd_linc_qc.csv` (XCP-D linc QC)
    ...
    -------
    None
        Writes `group_minimal_qc.csv` into the `derivatives/qc/xcpd` folder
    """
    DERIVATIVES_DIR = config["common"]["derivatives"]
    ...
    if qc_xcpd:
        ...
        path_to_group_qc = f"{DERIVATIVES_DIR}/qc/xcpd/group_xcpd_linc_qc.csv"
        group_qc.to_csv(path_to_group_qc, index=False)
